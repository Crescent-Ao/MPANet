...
def mkdir_exp(property):
    """
    :param property: 实验数据的地址
    :return:
    """
    property_path = property
    property_path = os.path.join(os.getcwd(), property_path)
    if (not os.listdir(property_path)):
        os.mkdir(os.path.join(property_path, 'exp1'))
        cur_sequence = 0
    else:
        cur_sequence = max(os.listdir(property_path))
        os.mkdir(os.path.join(property_path, 'exp' + str(int(cur_sequence[-1]) + 1)))
    return os.path.join(property_path, 'exp' + str(int(cur_sequence[-1]) + 1))
def save_ckpt(epoch,ckpt_path,trainer):
    save_path = os.path.join(os.path.join(ckpt_path,"ckpt_%s.pth"%(str(epoch))))
    print('current_epoch',epoch)
    checkpoint = {"net": trainer.model.state_dict(),
                  "optimizer": trainer.model.state_dict(),
                  'epoch': epoch,
